...
const changePage = (direction) => {
    ...
    if (direction === 'next' && props.nextPageUrl) {
        ...
        queryParams.page = Number(props.currentPage) + 1;
...
<template>
    <div v-if="links.length > 2"
        ...
        <div class="flex items-center font-medium text-[#697077] tracking-wider">
            <span>{{ (+props.currentPage - 1) * +perPageValue + 1 }} - {{ Math.min(+props.currentPage * +perPageValue,
                props.totalCount) }} из {{ totalCount }}</span>
        </div>
        ...
        <div class="flex">
            ...
            <div class="flex items-center ml-3 lg:ml-5">
                <!-- Previous button -->
                <button class="text-sm leading-4 text-[#697077] border border-current rounded" type="button"
                    :disabled="!props.currentPage || props.currentPage <= 1" @click="changePage('prev')">
                    <svg class="block w-5 h-5 lg:w-6 lg:h-6" width="24" height="24" viewBox="0 0 24 24" fill="none"
                ...
                <!-- Next button -->
                <button class="text-sm leading-4 text-[#697077] border border-current rounded"
