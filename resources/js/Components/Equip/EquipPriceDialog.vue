...
const formatted_agents = computed(() => {
    return $props.agents.map(l=>({ title: l.name, value: l.id }));
});
...
const is_submit_disabled = computed(() => {
    if ($props.purpose === 'edit') {
        ...
        return curr.store_date == date?.value || curr.agent_id == agent?.value.value
                || curr.notes == notes?.value || curr.store_price == store_price?.value;

    } else if ($props.purpose === 'create') {
        return !date.value || !notes.value || !store_price.value || !operation_price.value;
    }
});

const title = computed(() => {
    return $props.purpose === 'create' ? 'Задать цену оборудования' : 'Редактировать цену оборудования';
});

watch(model, new_val => {
    if (!new_val || $props.purpose === 'create') return;

    date   .value = $props.price?.store_date;
    agent  .value = { title: $props.agents.find(l=>l.id==$props.contragent?.id)?.name, value: $props.contragent?.id };
...
async function submit() {
    router.post('/equip/price', {
        ...
        contragent_id: +agent.value.value,
                ...
                >
                    ...
                    <form class="mt-8 space-y-4" @submit="submit">
                        <UiFieldDate
                            v-model="date"
                            label="Дата"
                            :inp-attrs="{ placeholder: 'Укажите дату' }"
                        />
                        <UiFieldSelect v-model="agent" :items="formatted_agents" label="Заказчик" />
