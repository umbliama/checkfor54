...
const updateUrl = () => {
    ...
    Object.keys(params).length && router.get(route('equip.tests', params));
};
...
function openDialog(test_id) {
    test_to_edit.value = { ...tests.value.find(t => t.id == test_id) };
    is_dialog_open.value = true;
}

const closeTest = (id, on_test_date, test_date, location_id, expense, description, isLocked) => {
    router.post(`/equip/tests/close/${id}`, {
        'on_test_date': on_test_date,
        'test_date': test_date,
...
onMounted(() => {
    ...
    updateTestTable(fetch_by.category_id, fetch_by?.size_id, fetch_by?.series);

    store.dispatch('equipment/updateMenuItem', EquipMenuItems.TESTS);
})
...
<template>
    <AuthenticatedLayout>
        ...
        <div class="p-5">
            ...
            <div class="flex justify-between bg-my-gray mt-4 p-1 lg:hidden">
                <UiFieldSelect v-model="localSeriesActive" :items="get_equipmentSeries"
                    :trigger-attrs="{ class: 'bg-white' }" :disabled="!get_equipmentSeries.length" placeholder="Номер"
            ...
            <div v-if="selectedCategory" class="flex mt-5">
                ...
                <div v-if="seriesActive" class="w-full space-y-5 lg:w-[calc(100%-100px-14px)]">
                    <div class="font-bold text-gray1">Добавить новую запись испытания</div>
                    <div class="w-full max-w-full mt-2.5 bg-bg2 overflow-x-auto border border-gray3">
                        <div class="min-w-[1050px] text-xs">
                            <div
                                ...
                                <div
                                    class="shrink-0 flex items-center justify-center w-[calc(12.14%+44px)] py-2.5 px-2">
                                    Дата
                                    испытания</div>
                                <div class="shrink-0 flex items-center justify-center w-[12.14%] py-2.5 px-2">Испытания
                                </div>
                            ...
                            <div
                                ...
                                <div
                                    class="shrink-0 flex items-center w-[calc(100%-12.14%-44px-12.14%-8.94%-8.94%-50px)]">
                    ...
                    <template v-if="tests">
                        <div class="mt-20 font-bold text-gray1">Стендовые испытания</div>
                        <div class="w-full max-w-full mt-2.5 bg-bg2 overflow-x-auto border border-gray3">
                            <div class="min-w-[1050px] text-xs">
                                <div
                                    class="flex font-bold border-b border-b-gray3 [&>*:not(:first-child)]:border-l [&>*:not(:first-child)]:border-l-gray3">
                                    ...
                                    <div class="shrink-0 flex items-center w-[8.94%] py-2.5 px-2">Место проведения</div>
                                    <div
                                        class="shrink-0 flex items-center w-[calc(100%-12.14%-44px-12.14%-8.94%-8.94%-110px)] py-2.5 px-2">
                                        Описание</div>
                                    <div class="shrink-0 flex items-center justify-center w-[8.94%] py-2.5 px-2">Расход
                                    </div>
                                ...
                                <template v-for="test in tests">
                                    <div
                                        class="flex border-b border-b-gray3 [&>*:not(:last-child)]:border-r [&>*:not(:last-child)]:border-r-gray3 break-all hover:bg-bg1">
                                        <div class="shrink-0 flex items-center justify-center w-[44px] py-2.5 px-2">
                                            <UiHyperlink :item-id="test.id" :hyperlink="test.hyperlink"
                                        ...
                                        <div class="shrink-0 flex items-center justify-center w-[12.14%] py-2.5 px-2">
                                            {{ test.test_date || '-' }}</div>
                                        <div class="shrink-0 flex items-center w-[8.94%] py-2.5 px-2">{{
                                        ...
                                        <div class="shrink-0 flex items-center w-[110px] py-2.5 px-2">
                                            ...
                                            <PopoverRoot v-else>
                                                ...
                                                <PopoverPortal>
                                                    <PopoverContent side="bottom" align="end"
                                                        ...
                                                        <div v-for="file in test.directory.files"
                                                        ...
                                                        <Link :href="'/directory/tests/' + test.id"
                                            ...
                                            <DropdownMenuRoot>
                                                ...
                                                <DropdownMenuPortal>
                                                    <transition name="fade">
                                                        <DropdownMenuContent v-if="!test.isLocked || $page.props.user.isAdmin"
                                                            class="py-2 px-1.5 rounded-md font-medium text-sm bg-white text-[#464F60] shadow-[0px_0px_0px_1px_rgba(152,_161,_179,_0.1),_0px_15px_35px_-5px_rgba(17,_24,_38,_0.2),_0px_5px_15px_rgba(0,_0,_0,_0.08)]"
                                                            :side-offset="5" align="end">
                                                            <DropdownMenuItem >
                                                                <button type="button"
                                                                    class="inline-flex items-center py-1 px-2 rounded hover:bg-my-gray transition-all"
                                                                    @click="openDialog(test.id)">
                                                                    Редактировать
                                                                    <svg class="block ml-2" width="16" height="16"
                                                                        viewBox="0 0 16 16" fill="none"
                                                                        xmlns="http://www.w3.org/2000/svg">
        ...
        <EquipTestEditDialog v-model="is_dialog_open" :locations="props.equipment_location" :test="test_to_edit" />
    </AuthenticatedLayout>
