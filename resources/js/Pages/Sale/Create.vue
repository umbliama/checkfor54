...
const incRowSubEquip = (id) => {
    store.dispatch('sale/updateIncRowsInEquip', id);
}
        ...
        <DropdownMenuPortal>
            <transition name="fade">
                <DropdownMenuContent
                    ...
                    <DropdownMenuItem>
                        <Link :href="route('services.destroy', selectedEquipmentService.id)" method="DELETE"
...
<div v-for="(item, index) in subEquipment" :key="index" v-if="subEquipment.length > 0"
    ...
    <div class="shrink-0 flex items-center w-[44px] py-2.5 px-2">
        <UiHyperlink v-if="selectedEquipmentService" :hyperlink="selectedEquipmentService?.hyperlink"
            :item-id="selectedEquipmentService?.id" endpoint="/services" />
    </div>
    <div class="shrink-0 flex items-center w-[15.84%] !border-l-violet-full">
        <button v-if="!subEquipment" class="block w-full h-full px-2 bg-transparent" @click="openDialog">
    ...
    <div class="shrink-0 flex items-center w-[14.08%] cursor-pointer">
        <input type="date" class="block w-full h-full px-2 bg-transparent" onclick="this.showPicker()"
            @change="updateByKey(index,'subequipment_id',item.id)"
            @input="updateByKey(index, 'shipping_date', $event.target.value)" />
    </div>
    <div class="shrink-0 flex items-center w-[calc(100%-44px-15.84%-14.08%-14.08%-100px)]">
        <input v-model="item.commentary" @input="updateByKey(index, 'commentary', $event.target.value)" type="text"
            class="block w-full h-full px-2 bg-transparent" />
    </div>
    <div class="shrink-0 flex items-center w-[14.08%]">
        <input v-model="item.price" @input="updateByKey(index, 'price', $event.target.value)" type="text"
            class="block w-full h-full px-2 bg-transparent" />
    </div>
    <div class="shrink-0 flex items-center w-[100px] py-2.5 px-2">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <DropdownMenuPortal>
                <transition name="fade">
                    <DropdownMenuContent
                        ...
                        <DropdownMenuItem>
                            <Link :href="route('services.destroy', item.id)" method="DELETE"
                                class="inline-flex items-center py-1 px-2 rounded text-danger hover:bg-my-gray transition-all">
                            Удалить
                            <svg class="block ml-2" width="16" height="16" viewBox="0 0 16 16" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
...
<div v-if="subRowsCount > 0" v-for="item in subRowsCount"
    class="flex font-bold border-b border-b-gray3 [&>*:not(:first-child)]:border-l [&>*:not(:first-child)]:border-l-gray3">
    <button class=" text-side-gray-text px-4 py-2 rounded" @click="is_dialog_open = true">
        Нажмите чтобы выбрать оборудование
    </button>
</div> -->
                            ...
                            <AccordionItem  v-for="(equipment,mainIndex) in selectedEquipment" :value="'item-'+equipment.id">
                                <AccordionHeader>
                                    <div
                                        ...
                                        <div class="shrink-0 flex items-center w-[15.84%] py-2.5 px-2">
                                            {{ equipment.category.name }} {{ equipment.size.name }} {{ equipment.series
                                            }}
