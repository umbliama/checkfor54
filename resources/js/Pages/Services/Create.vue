                ...
                <DropdownMenuContent
                    ...
                    <div v-for="(equipment,index) in selectedEquipmentService">
                        <div
                            ...
                            <div class="shrink-0 flex items-center w-[15.84%] py-2.5 px-2">{{ equipment.category.name }}
                                {{ equipment.size.name }} {{ equipment.series }}</div>
                            <div class="shrink-0 flex items-center w-[14.08%] cursor-pointer">
                                <input @input="updateEquipmentByKey(index, 'shipping_date', $event.target.value)"  v-model="form.shipping_date" type="date"
                                    class="block w-full h-full px-2 bg-transparent" onclick="this.showPicker()" />
                            </div>
                            <div class="shrink-0 flex items-center w-[14.08%]">
                                <input @input="updateEquipmentByKey(index, 'period_start_date', $event.target.value)" v-model="form.period_start_date" type="date"
                                    class="block w-full h-full px-2 bg-transparent" onclick="this.showPicker()" />
                            </div>
                            <div class="shrink-0 flex items-center w-[calc(100%-44px-15.84%-14.08%-14.08%-100px)]">
                                <input @input="updateEquipmentByKey(index, 'commentary', $event.target.value)"  v-model="form.commentary" type="text"
                                    class="block w-full h-full px-2 bg-transparent" />
                            </div>
                            <div class="shrink-0 flex items-center w-[100px] py-2.5 px-2">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                        ...
                        <div>
                            <div v-for="(subEquipment, subindex) in equipment.subEquipment"
                                ...
                                <div class="shrink-0 flex items-center w-[14.08%] cursor-pointer">
                                    <input
                                    @change="updateSubEquipmentByIndex(index,subindex,{'subequipment_id':subEquipment.id, 'shipping_date': $event.target.value})"
                                     type="date" class="block w-full h-full px-2 bg-transparent"
                                        onclick="this.showPicker()" />
                                </div>
                                <div class="shrink-0 flex items-center w-[14.08%]">
                                    <input  @change="updateSubEquipmentByIndex(index,subindex,{'period_start_date': $event.target.value})"
                                     type="date" class="block w-full h-full px-2 bg-transparent"
                                        onclick="this.showPicker()" />
                                </div>
                                <div class="shrink-0 flex items-center w-[calc(100%-44px-15.84%-14.08%-14.08%-100px)]">
                                    <input type="text"
                                    @change="updateSubEquipmentByIndex(index,subindex,{'commentary': $event.target.value})"
                                        class="block w-full h-full px-2 bg-transparent" />
                                </div>
                                <div class="shrink-0 flex items-center w-[100px] py-2.5 px-2">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            ...
                            <div v-if="subRowsCount >  selectedEquipmentService.length + subEquipment.length "
                                ...
                                <div :class="{ '!border-l-violet-full': equipmentType === 1 }"
                                    ...
                                    <button class="block w-full h-full px-2 bg-transparent text-left"
                                        @click="openDialog">
                                        Нажмите чтобы выбрать оборудование {{ selectedEquipmentService.length + subEquipment.length + 2 }}
                                    </button>
                                </div>
                                <div class="shrink-0 flex items-center w-[14.08%] cursor-pointer">
                                    <input type="date" class="block w-full h-full px-2 bg-transparent"
