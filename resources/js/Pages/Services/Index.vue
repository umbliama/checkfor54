...
const findContragentData = (id) => {
    return props.contragentsServiceData.find(eq => eq.id === id) || {};
};

const contragentData = reactive({
    'commentary': null,
    'contragent_id': null,
    'shipping_date': null
})
...
const calcFullIncome = () => {
    ...
    for (const key in raw) {
        if (raw.hasOwnProperty(key)) {
            ...
            for (const service of servicesArray.services) {
...
const setContragentId = (id) => {
    contragentId.value = id;
}

const submitContragentData = () => {
    router.post('/service/setContragentServiceData', {
        'contragent_id': Number(contragentId.value),
        'commentary': contragentData.commentary,
        'shipping_date': contragentData.shipping_date,
    })
    edit_dialog_open.value = false;
}
                                            ...
                                            <transition name="fade">
                                                <DropdownMenuContent
                                                    ...
                                                    <DropdownMenuItem>
                                                        <button type="button"
                                                            class="inline-flex items-center py-1 px-2 rounded hover:bg-my-gray transition-all"
                                                            @click="openEditDialog(service.services[0].contragent_id)">
                                                            Редактировать
                                    ...
                                    <AccordionItem v-for="(service_item, index) in service.services"
                                        ...
                                        <AccordionHeader
                                            ...
                                            <div class="shrink-0 flex items-center justify-center w-[44px] py-2.5 px-2">
                                                <UiHyperlink :item-id="service_item.id"
                                                    :hyperlink="service_item.hyperlink" endpoint="/equipment/service" />
                                            </div>
                                            <div
                                            ...
                                            <div class="shrink-0 flex items-center w-[14.08%]">
                                                <input v-model="service_item.service_date" type="date"
                                                    class="block w-full h-full px-2 bg-transparent" />
                                            </div>
                                            <div
                                                class="shrink-0 flex items-center w-[calc(100%-44px-15.84%-14.08%-14.08%-100px)]">
                                                <input v-model="service_item.commentary" type="text"
                                                    class="block w-full h-full px-2 bg-transparent" />
                                            </div>
                                                    ...
                                                    <AccordionHeader
                                                        ...
                                                        <div
                                                            ...
                                                            <UiHyperlink :item-id="subservice.equipment.id"
                                                                :hyperlink="subservice.equipment.hyperlink"
                                                                endpoint="/equipment" />
                                                        </div>
                                                        <div
                                                        ...
                                                        <div v-for="(sub, index) in subservice.service_subs"
                                                            ...
                                                            <div
                                                                ...
                                                                <UiHyperlink :item-id="sub.equipment.id"
                                                                    :hyperlink="sub.equipment.hyperlink"
                                                                    endpoint="/equipment" />
                                                            </div>
                                                            <div
                ...
                <transition name="fade">
                    <DialogContent
                        ...
                        <div class="mt-8 space-y-4">
                            <UiFieldDate v-model="contragentData.shipping_date" label="Дата отгрузки"
                                :inp-attrs="{ placeholder: 'Укажите дату отгрузки' }" />
                            <UiField v-model="contragentData.commentary" label="Комментарий" textarea />

                            <button @click="submitContragentData"
                                class="w-full mt-4 bg-my-gray text-side-gray-text font-bold px-6 py-3 disabled:opacity-40">Сохранить</button>
                        </div>
                    </DialogContent>
                </transition>
            </DialogPortal>
