    ...
    <div class="container md:mx-auto sm:mx-auto">
      ...
      <div class="flex">
        ...
        <div class="flex-1 sm:w-full">
              ...
              <div class="max-w-screen-xl px-4 py-3">
                <div
                  ...
                  <ul
                    ...
                    <li>
                      <div class="flex" :class="{ 'border-b-2 border-blue-600': selectedActive }">
                        <Link @click.prevent="selectActive(true)" class="text-lg  ">Открытые аренды</Link>
                        <span class="ml-1 rounded-full text-sm flex items-center px-2 py-1 text-white bg-gray-500 ">
                    ...
                    <Menu as="div" class="relative inline-block text-left">
                      ...
                      <transition enter-active-class="transition ease-out duration-100"
                        ...
                        <MenuItems
                          class="sm:fixed right-0 z-10 mt-2 w-60 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                          <div class="py-1">
          ...
          <nav class="bg-my-gray rounded-lg mt-4">
            <div class="max-w-screen-xl py-2">
              <div class="mt-6 flex items-center">
                <ul
                  class="flex overflow-x-auto sm:px-2 flex-row pb-3 border-b-2 border-gray-200 font-medium mt-0 space-x-8 rtl:space-x-reverse text-sm">

                  <li :class="{ 'border-b-2 border-blue-600  ': getMonth === 'all' }"
                    class="text-my-nav-text text-md w-full" @click="updateMonth('all')">
                    Все
                  </li>
                  <li @click="updateMonth('jan')" :class="{ 'border-b-2 border-blue-600  ': getMonth === 'jan' }"
                    class="text-my-nav-text text-md w-full">
                    Январь
                  </li>
                  <li @click="updateMonth('feb')" :class="{ 'border-b-2 border-blue-600  ': getMonth === 'feb' }"
                    class="text-my-nav-text text-md w-full">
                    Февраль
                  </li>
                  <li @click="updateMonth('mar')" :class="{ 'border-b-2 border-blue-600  ': getMonth === 'mar' }"
                    class="text-my-nav-text text-md w-full">
                    Март
                  </li>
                  <li @click="updateMonth('apr')" :class="{ 'border-b-2 border-blue-600  ': getMonth === 'apr' }"
                    class="text-my-nav-text text-md w-full">
                    Апрель
                  </li>
                  <li @click="updateMonth('may')" :class="{ 'border-b-2 border-blue-600  ': getMonth === 'may' }"
                    class="text-my-nav-text text-md w-full">
                    Май
                  </li>
                  <li @click="updateMonth('jun')" :class="{ 'border-b-2 border-blue-600  ': getMonth === 'jun' }"
                    class="text-my-nav-text text-md w-full">
                    Июнь
                  </li>
                  <li @click="updateMonth('jul')" :class="{ 'border-b-2 border-blue-600  ': getMonth === 'jul' }"
                    class="text-my-nav-text text-md w-full">
                    Июль
                  </li>
                  <li @click="updateMonth('aug')" :class="{ 'border-b-2 border-blue-600  ': getMonth === 'aug' }"
                    class="text-my-nav-text text-md w-full">
                    Август
                  </li>
                  <li @click="updateMonth('sept')" :class="{ 'border-b-2 border-blue-600  ': getMonth === 'sept' }"
                    class="text-my-nav-text text-md w-full">
                    Сентябрь
                  </li>
                  <li @click="updateMonth('oct')" :class="{ 'border-b-2 border-blue-600  ': getMonth === 'oct' }"
                    class="text-my-nav-text text-md w-full">
                    Октябрь
                  </li>
                  <li @click="updateMonth('novb')" :class="{ 'border-b-2 border-blue-600  ': getMonth === 'novb' }"
                    class="text-my-nav-text text-md w-full">
                    Ноябрь
                  </li>
                  <li @click="updateMonth('dec')" :class="{ 'border-b-2 border-blue-600  ': getMonth === 'dec' }"
                    class="text-my-nav-text text-md w-full">
                    Декабрь
                  </li>
                </ul>
          ...
          <div class="overflow-x-auto pb-20">
            <!-- Table header -->
            <div class="min-w-full whitespace-nowrap grid grid-cols-11 gap-52 bg-gray-100 font-bold p-2 rounded-md">
              <div>Оборудование</div>
              <div>Дата отгрузки</div>
              <div>Начало периода</div>
              <div>Хранение</div>
              <div>Наработка</div>
              <div>Окончание периода</div>
            ...
            <div v-if="filterDatesActive().length && selectedActive" class="">
              <div v-for="(service, index) in filterDatesActive()" :key="service.id"
                ...
                <!-- Main service row -->
                <div class="min-w-full grid grid-cols-11 gap-52 items-center whitespace-nowrap">
                  ...
                  <div>{{ service.period_start_date }}</div>
                  <div>{{ service.store ?? "Не задано" }} </div>
                  <div>{{ service.operating ?? "Не задано" }}</div>
                  <div>{{ service.period_end_date ?? "Не задано" }}</div>
                  <div>{{ service.return_date ?? "Не задано" }}</div>
                  <div>{{ service.return_reason ?? "Не задано" }}</div>
                  <div>{{ service.income ?? "Не задано" }}</div>
                  <!-- Actions column (empty for now) -->
                  <div class="flex justify-center items-center space-x-2">
                    <!-- Placeholder for future SVGs or buttons -->
                  </div>
                </div>
