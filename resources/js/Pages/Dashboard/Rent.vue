...
const chooseAgent = (id) => {
    if (id === "all") {
        if (chosenAgents.value.length === props.rented_services_grouped.data.length) {
            ...
            chosenAgents.value = props.rented_services_grouped.data.map(item => item.contragent_id);
        }
    } else {
        const index = chosenAgents.value.indexOf(id);
        if (index === -1) {
            chosenAgents.value.push(id);
        } else {
            chosenAgents.value.splice(index, 1);
        }
    }
};
...
function calculateTotalIncome(rented_services_grouped) {
    return rented_services_grouped.data.reduce((totalSum, contragent) => {
        return totalSum + contragent.services.reduce((serviceSum, service) => {
            return serviceSum + service.service_equipment.reduce((equipSum, equipment) => {
                const mainIncome = equipment.income || 0;
                const subEquipmentsIncome = equipment.subservice_equipment.reduce(
                    (subSum, subItem) => subSum + (subItem.income || 0),
                    0
                );
                return equipSum + mainIncome + subEquipmentsIncome;
            }, 0);
        }, 0);
    }, 0);
}
...
const updateFiltersAndFetchData = () => {
    const { current_page, total, last_page } = props.rented_services_grouped;
...
const filteredServicesGrouped = computed(() => {
    if (!props.rented_services_grouped || !props.rented_services_grouped.data) {
        return props.rented_services_grouped;
    }

    let filteredData = props.rented_services_grouped.data;

    if (chosenAgents.value.length > 0) {
        filteredData = filteredData.filter(item => chosenAgents.value.includes(item.contragent_id));
    }

    const categoryFilter = chosenCategory.value ? Number(chosenCategory.value) : null;
    const sizeFilter = chosenSize.value ? Number(chosenSize.value) : null;
    const startDateFilter = start_date.value ? new Date(start_date.value) : null;
    const endDateFilter = end_date.value ? new Date(end_date.value) : null;

    if (categoryFilter || sizeFilter || startDateFilter || endDateFilter) {
        filteredData = filteredData.map(contragent => {
            let filteredServices = contragent.services
                .map(service => {
                    ...
                    let filteredEquipmentWithSub = service.service_equipment.map(equipment => {
                        let filteredSubs = equipment.subservice_equipment.filter(subEquip => {
                            ...
                            const matchesSize = sizeFilter ? subEquip.equipment.size_id === sizeFilter : true;
                            return matchesCategory && matchesSize;
                        });

                        return {
                            ...equipment,
                            subservice_equipment: filteredSubs
                        };
                    });
                    ...
                    if (filteredMainEquipment.length > 0 || filteredEquipmentWithSub.some(e => e.subservice_equipment.length > 0)) {
                        return {
                            ...service,
                            service_equipment: filteredMainEquipment.length > 0 ? filteredMainEquipment : filteredEquipmentWithSub
                        };
                    }
...
total_equipment_sizes}}</span>
                        ...
                        <li v-for="size in equipment_sizes"
                            :class="{ '!border-[#001D6C] text-[#001D6C]': chosenSize === size.id }"
                            class="flex items-center border-b-2 border-transparent py-3 cursor-pointer"
            ...
            <div class="block mt-4 lg:hidden">
                <div class="flex justify-between bg-my-gray p-1 text-sm lg:hidden">
                    <PopoverRoot>
                        <PopoverTrigger class="flex items-center w-full py-2 px-3 text-gray1 bg-white lg:py-3 lg:px-4">
            ...
            <div class="flex w-full mt-5">
                ...
                <div class="w-full lg:w-[calc(100%-168px-14px)]">
                    <div v-for="item in filteredServicesGrouped.data">
                        <div
                            class="items-center justify-between mb-2 py-3 px-2.5 text-base border border-[#EEEEF2] lg:flex">
                            <div class="font-bold text-xs">{{ item.contragent_name }}:</div>
                            <div class="mt-2 font-medium text-[#484964] lg:mt-0">
                                <span class="mr-3.5">Сумма</span>
                                {{ item.total_income }} ₽
                            </div>
                        </div>
                        <div class="space-y-6">
                            <AccordionRoot type="multiple" :collapsible="true">
                                <AccordionItem v-for="item in item.services" value="'head-item-'+index" class="mt-3">
                                    <AccordionHeader
                                        class="items-center justify-between py-3 px-2.5 text-base border border-[#EEEEF2] lg:flex">
                                        ...
                                        <div class="min-w-[1136px] text-xs">
                                            <div
                                                ...
                                                <div
                                                    ...
                                                    Комментарий</div>
                                                <div class="shrink-0 flex items-center w-[14.08%] py-2.5 px-2">Доход
                                                </div>
                                            ...
                                            <AccordionRoot type="multiple" :collapsible="true">
                                                <AccordionItem v-for="item in item.service_equipment" value="item-1">
                                                        ...
                                                        <div
                                                            ...
                                                            <div :class="{ 'bg-violet/5': true }"
                                                                ...
                                                                <AccordionTrigger
                                                                    ...
                                                                    <svg class="block" width="18" height="18"
                                                                        viewBox="0 0 18 18" fill="none"
                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                        <path
                                                                            d="M3.375 6C5.625 10.875 12.375 10.875 14.625 6"
                                                                            stroke="#644DED" stroke-width="1.5"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round" />
                                                                        <path d="M12.6122 8.4881L14.625 11.25"
                                                                            stroke="#644DED" stroke-width="1.5"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round" />
                                                                        <path d="M9 9.65625V12.375" stroke="#644DED"
                                                                            stroke-width="1.5" stroke-linecap="round"
                                                                            stroke-linejoin="round" />
                                                                        <path d="M5.38787 8.4881L3.375 11.25"
                                                                            stroke="#644DED" stroke-width="1.5"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round" />
                                                                    </svg>
                                                                    <svg class="block ml-1 transition-all" width="18"
                                                                        height="18" viewBox="0 0 18 18" fill="none"
                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                        <path d="M10.5 12L7.31802 8.81802L10.5 5.63604"
                                                                    ...
                                                                    <DropdownMenuPortal>
                                                                        <transition name="fade">
                                                                            <DropdownMenuContent
                                                                                ...
                                                                                <DropdownMenuItem>
                                                                                    <Link
                                                                                        :href="route('services.destroy', 12)"
                                                                                        method="delete"
                                                    ...
                                                    <AccordionContent v-for="sub in item.subservice_equipment"
                    ...
                    <Pagination :links="filteredServicesGrouped.links"
                        :total-pages="Math.ceil(filteredServicesGrouped.total / filteredServicesGrouped.per_page)"
                        :nextPageUrl="filteredServicesGrouped.next_page_url"
                        :prevPageUrl="filteredServicesGrouped.prev_page_url"
                        :totalCount="filteredServicesGrouped.last_page"
                        :currentPage="filteredServicesGrouped.current_page" />
                </div>
            </div>
        </div>
