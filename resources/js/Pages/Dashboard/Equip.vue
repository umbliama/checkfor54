...
const updateUrl = () => {
    ...
    Object.keys(params).length && router.get(route('dashequip', params));
};
...
<template>
    ...
    <AuthenticatedLayout>
        <DashboardToolbar dashboard-page-type="dashequip" />
                ...
                <div v-if="selectedCategory == 1"
                    ...
                    <div class="min-w-[1280px] text-xs">
                        <div class="flex font-bold border-b border-b-gray3 [&>*:not(:first-child)]:border-l [&>*:not(:first-child)]:border-l-gray3">
                            <div class="shrink-0 flex items-center w-[18.7%] py-2.5 px-2">Производитель</div>
                            <div class="shrink-0 flex items-center w-[7.86%] py-2.5 px-2">Заходность</div>
                            <div class="shrink-0 flex items-center w-[7.86%] py-2.5 px-2">Длина</div>
                            <div class="shrink-0 flex items-center w-[7.86%] py-2.5 px-2">Наработка</div>
                            <div class="shrink-0 flex items-center w-[7.5%] py-2.5 px-2">Изготовлен</div>
                            <div class="shrink-0 flex items-center w-[9.96%] py-2.5 px-2">Состояние</div>
                            <div class="shrink-0 flex items-center w-[calc(100%-18.7%-7.86%-7.86%-7.86%-7.5%-9.96%-50px)] py-2.5 px-2">Местонахождение</div>
                        ...
                        <template v-for="item in sortedEquipment">
                            ...
                            >
                                <div class="shrink-0 flex items-center w-[18.7%] py-2.5 px-2">
                                    <div class="mr-2">
                                        <UiHyperlink :item-id="item.id" :hyperlink="item.hyperlink"
                                            endpoint="/equipment" />
                                    </div>
                                    <span class="line-clamp-2">{{item.category.name}}{{ item.size.name }} {{item.series}}</span>
                                </div>
                                <span v-if="item.ownership === 'sub'" class="absolute left-9 top-0 w-px h-full bg-danger z-10 border-none"></span>
                                <div class="shrink-0 flex items-center w-[7.86%] py-2.5 px-2">{{ item.zahodnost ?? '-' }}</div>
                                <div class="shrink-0 flex items-center w-[7.86%] py-2.5 px-2">{{ item.size.name ?? '-' }}</div>
                                <div class="shrink-0 flex items-center w-[7.86%] py-2.5 px-2">{{ item.narabotka_ds }}</div>
                                <div class="shrink-0 flex items-center w-[7.5%] py-2.5 px-2">{{ item.manufactor_date ?? '-' }}</div>
                ...
                <div v-else
                    ...
                    <div class="min-w-[1144px] text-xs">
                       <div class="flex font-bold border-b border-b-gray3 [&>*:not(:first-child)]:border-l [&>*:not(:first-child)]:border-l-gray3">
                            ...
                            <div class="shrink-0 flex items-center w-[7.5%] py-2.5 px-2">Изготовлен</div>
                            <div class="shrink-0 flex items-center w-[9.96%] py-2.5 px-2">Состояние</div>
                            <div class="shrink-0 flex items-center w-[calc(100%-18.7%-7.86%-7.86%-7.5%-9.96%-50px)] py-2.5 px-2">Местонахождение</div>
                            <div class="shrink-0 flex items-center justify-center w-[50px] py-2.5 px-2">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd"
