...
class ServiceSeeder extends Seeder
{
    ...
    { {
            ...
            if (!$equipment || !$contragent) {
                $this->command->error('No Equipment or Contragent records found. Please seed these tables first.');
                return;
            }
            for ($i = 0; $i < 20; $i++) {
            ...
            for ($i = 0; $i < 3; $i++) {
                DB::table('service_subequipment')->insert([
                    ...
                    'shipping_date' => now()->addDays(2 + $i),
                    'period_start_date' => now()->addDays(3 + $i),
                    'commentary' => "Subequipment item $i commentary",
                    'return_date' => now()->addDays(12 + $i),
                    'period_end_date' => now()->addDays(22 + $i),
                    'store' => '20',
                    'operating' => '20',
                    'income' => rand(100, 500),
                    'return_reason' => $i % 2 === 0 ? 'project' : 'rejected',
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);
            }
